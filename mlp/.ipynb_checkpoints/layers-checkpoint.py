...
class AffineLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        x = inputs
        y = np.dot(x, self.weights.T) + self.biases
        return y
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        output_dim = grads_wrt_outputs.shape[1]
    
        grads_wrt_weights = np.zeros((output_dim, input_dim))
        grads_wrt_biases = np.zeros(output_dim)

        for n in range(batch_size):
            for k in range(output_dim):
                for j in range(input_dim):        
                    grads_wrt_weights[k][j] += grads_wrt_outputs[n][k] * inputs[n][j]
                grads_wrt_biases[k] += grads_wrt_outputs[n][k]

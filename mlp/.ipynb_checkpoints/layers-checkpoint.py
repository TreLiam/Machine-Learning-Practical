    ...
    def fprop(self, inputs, stochastic=True):
        ...
        if stochastic == True:
            if self.share_across_batch == True:
                mask = (self.rng.uniform(0,1,size=inputs.shape[1:]) < self.incl_prob).astype(float)
                for i in range(batch_size):
                    outputs[i] = mask * outputs[i]
            else:
                for i in range(batch_size):
                    mask = (self.rng.uniform(0,1,size=inputs.shape[1:]) < self.incl_prob).astype(float)
                    outputs[i] = mask * outputs[i]
        else:
            outputs = outputs * self.incl_prob

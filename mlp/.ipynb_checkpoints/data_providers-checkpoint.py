...
class MNISTDataProvider(DataProvider):
    ...
    def to_one_of_k(self, int_targets):
        ...
        """
        coded_targets = np.zeros((int_targets.shape[0], 9))
        for i in range(int_targets.shape[0]):
            coded_targets[i][int_targets[i] - 1] = 1
...
class MetOfficeDataProvider(DataProvider):
    ...
    def __init__(self, window_size, batch_size=10, max_num_batches=-1,
        ...
        loaded = loaded[loaded != -99.99].reshape(-1)
        ...
        windowed_row = int(int(loaded.shape[0] / self.window_size) * self.window_size)
        windowed_loaded = loaded[:windowed_row].reshape(-1, self.window_size)

        #TODO: separate into inputs and targets
        # inputs are the first (window_size - 1) entries in windows
        inputs = windowed_loaded[:,:-1]
        # targets are the last entries in windows
        targets = windowed_loaded[:,-1]

...
class EMNISTDataProvider(DataProvider):
    ...
    def label_smoothing(self, int_targets, alpha=0.1):
        ...
        """
        one_hot = np.zeros((int_targets.shape[0], self.num_classes))
        one_hot[np.arange(int_targets.shape[0]), int_targets] = 1

        soft_targets = np.where(one_hot == 1, 1 - alpha, alpha / (self.num_classes - 1))
        return soft_targets
